...
// 添加帧重组相关的成员变量声明
static std::vector<uint8_t> partial_frame_buffer_;  // 部分帧缓冲区
static uint32_t last_frame_byte_time_ = 0;  // 最后帧字节接收时间
static const uint32_t FRAME_TIMEOUT_MS = 500;  // 帧超时时间（毫秒）
...
void HLKLD2402Component::setup() {
  ...
  parent->set_baud_rate(115200);  // 设置波特率为 115200
  parent->set_stop_bits(1);  // 设置停止位为 1
  parent->set_data_bits(8);  // 设置数据位为 8
  parent->set_parity(esphome::uart::UART_CONFIG_PARITY_NONE);  // 设置无校验
  ...
  // 移除立即的固件版本检查 - 它将在 60 秒后发生
  // get_firmware_version_();
  
  // 设置默认版本 - 这将显示直到我们可以确定实际版本
...
void HLKLD2402Component::begin_passive_version_detection_() {
  ...
  firmware_version_ = "HLK-LD2402"; // 默认回退版本
}
...
// 更新 get_firmware_version_ 方法以使用正确的命令和解析
void HLKLD2402Component::get_firmware_version_() {
  ...
  // 根据协议规范 5.2.1 - 使用命令 0x0000 获取固件版本
...
void HLKLD2402Component::loop() {
  ...
  static const uint32_t PROCESS_INTERVAL = 2000; // 每2秒处理一次行数据
  static const uint32_t TIMEOUT_MS = 100; // 如果100ms没有数据，则重置缓冲区
  ...
  // 添加定期调试消息 - 减少频率
  if (millis() - last_debug_time > 30000) {  // 每30秒
  ...
  while (available()) {
    ...
    // 首次检查：检查数据帧头 (F4 F3 F2 F1)
    if (c == DATA_FRAME_HEADER[0]) {
      // 需要检查这是否真的是数据帧
      if (available() >= 4) { // 需要至少4个更多字节来验证帧头和帧类型
        // 预览接下来的4个字节以检查帧头和类型
        bool is_data_frame = true;
        uint8_t peek_bytes[4]; // 头 + 帧类型
        
        // 读取接下来的4个字节而不将它们从缓冲区中移除
        for (int i = 0; i < 4; i++) {
          if (!available()) {
            is_data_frame = false;
            break;
        ...
        if (is_data_frame) {
          // 我们有一个正确的数据帧头！收集整个帧
          // 第4个字节是帧类型 (0x83 表示距离数据，0x84 表示工程数据)
  ...
  if (calibration_in_progress_ && calibration_progress_sensor_ != nullptr) {
    uint32_t now = millis();
    if (now - last_calibration_check_ >= 5000) { // 每5秒检查一次
      ...
      if (send_command_(CMD_GET_CALIBRATION_STATUS)) {
        ...
        if (read_response_(response)) {
          ...
          bool handled = false; // 跟踪我们是否处理了响应
          
          // 处理与文档不同的实际设备响应格式
          // 预期格式：06 00 0A 01 00 00 XX 00 - 其中 XX 是进度值
          if (!handled && response.size() >= 8) {
            ...
            if (response[0] == 0x06 && response[1] == 0x00 &&
              ...
              // 我们成功处理了响应
              handled = true;  // 标记为已处理，而不是使用continue
            }
          }
  ...
  if (frame_data.size() >= 9) {
    ...
    if (!throttled) {
      ...
      if (this->distance_sensor_ != nullptr) {
        ...
        if (distance_change_threshold_ > 0 && last_published_distance_ >= 0) {
          float distance_diff = abs(target_distance - last_published_distance_);
          if (distance_diff < distance_change_threshold_) {
        ...
        if (should_publish) {
          ...
          last_published_distance_ = target_distance;
        }
      }
...
void HLKLD2402Component::process_line_(const std::string &line) {
  ...
  if (valid_distance) {
    ...
    if (!throttled && this->distance_sensor_ != nullptr) {
          ...
          bool should_publish = true;
          if (distance_change_threshold_ > 0 && last_published_distance_ >= 0) {
            float distance_diff = abs(distance_cm - last_published_distance_);
            if (distance_diff < distance_change_threshold_) {
              should_publish = false;
            }
          }
          ...
          if (should_publish) {
            ...
            last_published_distance_ = distance_cm;
          }
        }
  }
}
